...
netstat -ltnp 2>/dev/null | grep -q ":8080 " && { echo "Port 8080 is busy, pkill -9 fast-time-server may help"; exit 1; }
...
curl -N "$URL" "${HEADERS[@]}" -d "$INIT" -D ~/tmp/fast-time-server-headers.txt
SESSION_ID=$(grep -i "mcp-session-id" ~/tmp/fast-time-server-headers.txt | cut -d' ' -f2 | tr -d '\r')
